-- Run this ONCE in the Supabase SQL Editor. Makes daily/weekly/monthly reflections
-- visible to shared (no-login) viewers, EXCEPT lines that begin with // which stay
-- private to you. Safe on an existing database; a fresh schema.sql already has it.

-- remove lines that start (after optional spaces) with // — these stay private to the owner
...
returns text language sql immutable as $$
  select case when t is null then null else
    nullif(
      array_to_string(
        array(
          select ln from unnest(string_to_array(t, E'\n')) ln
...
begin
  ...
  -- reflections (private // lines stripped)
  select json_build_object(
